function sfsi_validationStep2() {
    ...
    if (sfsi_validator(SFSI('input[name="sfsi_youtube_page"]'), "activte") && sfsi_validator(SFSI('input[name="sfsi_youtube_follow"]'), "checked")) {
        if(jQuery("input[name='sfsi_youtubeusernameorid']:checked").val()== 'name')
		{
		...
		{
			if (!sfsi_validator(SFSI('input[name="sfsi_ytube_chnlid"]'), "blank")) {
				showErrorSuc("error", "Error : Invalid youtube channel id", 2);
				SFSI('input[name="sfsi_ytube_chnlid"]').addClass("inputError");
				return false
			}
		}
    }
...
function sfsi_validator(e, t) {
    var n = new RegExp("^(http|https|ftp)://([a-zA-Z0-9.-]+(:[a-zA-Z0-9.&%$-]+)*@)*((25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]).(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]|0).(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]|0).(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[0-9])|([a-zA-Z0-9-]+.)*[a-zA-Z0-9-]+.(com|edu|gov|int|mil|net|org|biz|arpa|info|name|pro|aero|coop|museum|[a-zA-Z]{2}))(:[0-9]+)*(/($|[a-zA-Z0-9.,?'\\+&%$#=~_-]+))*$");
    switch (t) {
        case "blank":
            if (!e.val().trim()) return false;
            else return true;
            break;
        case "url":
            if (!n.test(e.val().trim())) return false;
            else return true;
            break;
        case "checked":
            if (!e.attr("checked") === true) return false;
            else return true;
            break;
        case "activte":
            if (!e.attr("disabled")) return true;
            else return false;
            break;
        case "int":
            if (!isNaN(e.val())) return true;
            else return false;
            break
    }
}